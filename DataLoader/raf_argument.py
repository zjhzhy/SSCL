...
class RAFDataset(Dataset):
    ...
    def __prepare__(self, data_root, train, align):
        if not os.path.isdir(data_root):
            raise Exception
        images = []
        txt_path = os.path.join(data_root, 'list_patition_label.txt')
        ...
        for (s, label) in zip(data[0], data[1]):
            if align:
                ...
                path = os.path.join(data_root, 'aligned', s)
            else:
                path = os.path.join(data_root, 'original', s)
...
def get_dataset(args):
    data_root = '/home/lab303/zhy/data/RAF/RAFDB'
...
if __name__ == '__main__':
    get_distribution(data_root='/home/lab303/zhy/data/RAF/RAFDB')
