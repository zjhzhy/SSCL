...
def pose_reader(filelist, num_classes):
    if num_classes == 7:
        exclude_list = [7, 8, 9, 10]
    else:
        exclude_list = [8, 9, 10]
...
class ImageList(data.Dataset):
    def __init__(self, root, fileList, train=True, num_classes=7, list_reader=default_reader,
                 loader=PIL_loader, argument=False, argu_type=0, scale_invariant=False):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        imgPath = os.path.join(self.root, label,imgPath)
...
def get_dataloader(args):
    ...
    pose45_data = ImageList(root=vaild_path, train=False, fileList=pose45_list, num_classes=args.num_classes,
                            list_reader=pose_reader, argument=True, argu_type=args.argu_type,
                            scale_invariant=args.scale_invariant)
    pose30_list = "/home/lab303/yx/database/affectnet/pose_30_affectnet_list.txt"
    pose30_data = ImageList(root=vaild_path, train=False, fileList=pose30_list, num_classes=args.num_classes,
                            list_reader=pose_reader, argument=True, argu_type=args.argu_type,
                            scale_invariant=args.scale_invariant)
    occlusion_list = "/home/lab303/yx/database/affectnet/occlusion_affectnet_list.txt"
    occlusion_data = ImageList(root=vaild_path, train=False, fileList=occlusion_list, num_classes=args.num_classes,
                               list_reader=pose_reader, argument=True, argu_type=args.argu_type,
                               scale_invariant=args.scale_invariant)
    ...
    val_data = ImageList(root=vaild_path, train=False, fileList=valid_list, num_classes=args.num_classes,
                         argument=True, argu_type=args.argu_type, scale_invariant=args.scale_invariant)

    val_loader = torch.utils.data.DataLoader(val_data, args.batch_size, shuffle=False, num_workers=args.worker,
                                             pin_memory=True)

    train_dataset = ImageList(root=train_path, train=True, fileList=train_list, num_classes=args.num_classes,
                              argument=True, argu_type=args.argu_type, scale_invariant=args.scale_invariant)
