...
class ImageList(data.Dataset):
    ...
    def transform_path(self, path, target):
        ...
        if self.train:
            path = os.path.join(self.root, 'train', expression, fine_name)
        else:
            path = os.path.join(self.root, 'test', expression, fine_name)
        return path
...
if __name__ == '__main__':
    ...
    valid_list = "/home/lab303/yx/database/affectnet/validation.csv"
    train_list = "/home/lab303/yx/database/affectnet/training.csv"
    a = ImageList(root=root_path, train=True, fileList=train_list, num_classes=7,
                  argument=True, argu_type=1, scale_invariant=0)
    train_loader = torch.utils.data.DataLoader(a, 10, shuffle=False,
                                               num_workers=1, sampler=None,
                                               drop_last=True)
    for img, label, aru_img in train_loader:
        print(label)
        break
