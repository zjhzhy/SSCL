...
import argparse
import numpy as np
from DivideMix import pureResnet
from sklearn.mixture import GaussianMixture
import DivideMix.dataloader_raf as dataloader
import torchnet
...
parser.add_argument('--id', default='original', type=str)
...
def create_model():
    model = pureResnet.pure_resnet18(num_classes=args.num_class)
    model = model.cuda()
    return model


stats_log = open('./DivideMix/checkpoint/%s' % (args.id) + '_stats.txt', 'w')
test_log = open('./DivideMix/checkpoint/%s' % (args.id) + '_acc.txt', 'w')

warm_up = 1

loader = dataloader.raf_dataloader(batch_size=args.batch_size, num_workers=5, root_dir=args.data_path,
...
for epoch in range(args.num_epochs + 1):
    ...
    print("\n| Test Epoch #%d\t raf Acc: %.2f%% (%.2f%%) \n" % (
        epoch, web_acc[0], web_acc[1]))
    test_log.write('Epoch:%d \t raf Acc: %.2f%% (%.2f%%) \n' % (
        epoch, web_acc[0], web_acc[1]))
    test_log.flush()
    ...
    torch.save(all_loss, './DivideMix/checkpoint/%s.pth.tar' % (args.id))
