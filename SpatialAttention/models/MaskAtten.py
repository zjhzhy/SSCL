...
class MaskAttNet(nn.Module):
    def __init__(self, num_class=7, drop=0.0):
        super(MaskAttNet, self).__init__()
        ...
        self.drop = SpatialDropout(self.drop)
        self.sig = nn.Sigmoid()
        self.fc = nn.Linear(512, num_class)
    ...
    def forward(self, x):
        ...
        x = x * self.ca(x)
        att = self.sa(x)
        att = self.drop(att, noise_shape=(att.shape[0], att.shape[1], 1, 1))
        out = x * att
        # out = self.bn(out)
