...
import torch
import os
from SpatialAttention.models.TransFER_DAN import mymodel
import argparse
import utils
import pandas as pd
...
def main():
    ...
    """nni.variable(nni.quniform(0.01,0.06,0.02), name=args.lr)"""
    args.lr = args.lr
    logsuffix = ''
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        for i, batch in pbar:
            ...
            # loss computation
            cls_loss = criterion(logit, target) + criterion_pl(att)
            loss = cls_loss
            # backward
            optimizer.zero_grad()
            loss.backward()
