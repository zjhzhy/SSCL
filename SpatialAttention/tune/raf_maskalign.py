...
from SpatialAttention.models.MaskAttenAlignNet import MaskAttenAlignNet
import nni
...
def run_training(params):
    ...
    args.drop = params['drop']
    if torch.cuda.is_available():
        torch.backends.cudnn.benchmark = True
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.enabled = True

    model = MaskAttenAlignNet(drop=args.drop)
    model.to(device)
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        for (imgs, targets) in train_loader:
            ...
            out1, out2, out = model(imgs)

            loss = criterion_cls(out, targets)
            kl_loss = criterion_kl(out1, out2)
