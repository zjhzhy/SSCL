...
import torch
import os
from SpatialAttention.models.TransFER import TransFER
import argparse
import utils
import pandas as pd
...
def main():
    ...
    if 'MAD' in args.tune:
        logger.info('tuning MAD')
        """nni.variable(nni.choice(2,3), name=mad_m)"""
        mad_m = 2
        args.mad_m = int(mad_m)
        logsuffix = logsuffix + 'm_{}'.format(args.mad_m)

        """nni.variable(nni.quniform(0.6,0.9,0.1), name=args.p1)"""
        args.p1 = args.p1
        logsuffix = logsuffix + 'p1_{:.2f}'.format(args.p1)

    if 'attention' in args.tune:
        logger.info('tuning attention_type')
        """nni.variable(nni.choice('true', 'false'), name=use_ca)"""
        use_ca = ''
        if use_ca == 'true':
            args.use_ca = True
        else:
            args.use_ca = False
        """nni.variable(nni.choice('true', 'false'), name=use_sa)"""
        use_sa = ''
        if use_sa == 'true':
            args.use_sa = True
        else:
            args.use_sa = False
        logsuffix += 'ca_{}'.format(args.use_ca)
        logsuffix += 'sa_{}'.format(args.use_sa)
    ...
    best_acc_epoch = 0
    # load models
    model = TransFER(args).to(cfg.device)
