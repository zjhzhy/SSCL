...
import torch
import os
from LocalAlignNet.models.EMDAlignNetMulti import DeepEMD
import argparse
import utils
import pandas as pd
...
from DataLoader.raf_multiargument import get_dataloader
import json
...
def main():
    ...
    if 'alpha' in args.tune:
        ...
        """nni.variable(nni.quniform(0.4,1.0,0.2), name=args.alpha)"""
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        for i, batch in pbar:
            ...
            # forward
            logit, emd_dists, target, emd_dists2, emd_dists3 = model(batch)
            ...
            for dist in emd_dists:
                dist = dist.mean(0)
                emd_loss += dist
            for dist in emd_dists2:
                dist = dist.mean(0)
                emd_loss += dist
            for dist in emd_dists3:
                dist = dist.mean(0)
                emd_loss += dist
        ...
        if epoch % args.eval_freq == 0:
            ...
            with torch.no_grad():
                ...
                for i, batch in pbar:
                    logit, _, target, _, _ = model(batch)
                    criterion = nn.CrossEntropyLoss()
