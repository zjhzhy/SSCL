...
from DataLoader.ckplus import get_five_flods
import json
...
def main():
    ...
    logsuffix = ''
    # tune trail space
    if 'argu' in args.tune:
        """nni.variable(nni.choice(0,1), name=argu_type)"""
        argu_type = 0
        args.argu_type = int(argu_type)
        logsuffix = logsuffix + 'argutype{}'.format(args.argu_type)

    if 'pretrained' in args.tune:
        logger.info('tuning pretrained')
        print('tuning pretrained:{}'.format(args.pretrained))
    ...
    if 'pool_size' in args.tune:
        ...
        """nni.variable(nni.choice(5,6,7), name=args.pool_size)"""
    ...
    if 'target_layers' in args.tune:
        ...
        """nni.variable(nni.choice('4','3', '2'), name=args.target_layers)"""
    ...
    print(json.dumps(vars(args), separators=('\n', ':')))
    print('nni experiments id:{}'.format(str(nni.get_experiment_id())))
    print('nni sequence id:{}'.format(str(nni.get_sequence_id())))
    ...
    for i in range(5):
        train_loader, val_loader = get_five_flods(args, i)
        ...
        for epoch in range(0, 0 + args.max_epoch):
            ...
            for i, batch in pbar:
                if i == 0:
                    ...
                    print('first_batch_gap_time:{}'.format(first_batch_gap_time))
            ...
            if epoch % args.eval_freq == 0:
                ...
                epoch_time.stop()
                print(
                    'Test: --Accuracy:{:6.2f} --Average Loss: {:.4f} --Time: {}'.format(top1.avg, losses.avg,
                                                                                        epoch_time))
                ...
                print('>>>>>best_acc:{:.3f}------best_acc_epoch:{:03d}>>>>>'.format(best_acc, best_acc_epoch))
                first_batch_gap_time = time.time()
        best_acc_list.append(best_acc)
    acc_list = np.array(best_acc_list)
    best_acc_mean = acc_list.mean()
    best_acc_var = acc_list.var()
    cfg.logger.rename(metric=best_acc_mean)
    print('best_acc_mean:{},var:{}'.format(best_acc_mean, best_acc_var))
    """@nni.report_final_result(best_acc_mean)"""
    return best_acc_mean


if __name__ == '__main__':
    '''@nni.get_next_parameter()'''
    logger = logging.getLogger('emd_ckplus_five_flods')
    try:
        main()
