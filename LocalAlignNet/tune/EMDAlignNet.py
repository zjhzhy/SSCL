...
def main():
    ...
    if 'alpha' in args.tune:
        ...
        """nni.variable(nni.quniform(0.1,0.6,0.1), name=args.alpha)"""
        args.alpha = args.alpha

    # mkdir and set version
    cfg = Config.cfg
    file_name = utils.get_cur_file_name(__file__) + '_'.join(args.tune)
    cfg = utils.set_version(cfg, version=file_name, suffix=str(nni.get_sequence_id()), tmp=args.tmp, args=args)
