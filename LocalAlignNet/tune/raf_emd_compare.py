...
import torch
import os
from LocalAlignNet.models.EMDCompareModel import DeepEMD
import argparse
import utils
import pandas as pd
...
def main():
    ...
    parser.add_argument('--backbone', type=str, default='ms1m')
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        for i, batch in pbar:
            ...
            # forward
            logit, emd_logtis, target = model(batch)
            emd_logtis = emd_logtis[-1]
            labels = target.contiguous().view(-1, 1)
            mask = torch.eq(labels, labels.T).float().to(args.device)
            logits_mask = torch.scatter(
                torch.ones_like(mask),
                1,
                torch.arange(args.batch_size).view(-1, 1).to(args.device),
                0
            )
            mask = mask * logits_mask

            exp_logits = torch.exp(emd_logtis) * logits_mask
            log_prob = emd_logtis - torch.log(exp_logits.sum(1, keepdim=True))
            # compute mean of log-likelihood over positive
            mean_log_prob_pos = (mask * log_prob).sum(1) / mask.sum(1)
            emd_loss = mean_log_prob_pos.mean()
