...
def valid_pose30(model, val_loader, cfg, best_acc=0.0):
    ...
    valid_loss, valid_pose30_acc = losses.avg, top1.avg
    """@nni.report_intermediate_result(valid_pose30_acc)"""

    if valid_pose30_acc > best_acc:
        best_acc = valid_pose30_acc
    _print('>>>>>pose30_best_acc:{:.3f}------>>>>>'.format(best_acc))
    return best_acc


def valid_pose45(model, val_loader, cfg, best_acc=0.0):
    _print = cfg.logger.print_log
    model.eval()
    epoch_time = TimeMeter()
    losses = AverageMeter(':.4f')
    ...
    _print(
        'Test: --Accuracy:{:6.2f} --Average Loss: {:.4f} --Time: {}'.format(top1.avg, losses.avg, epoch_time))
    valid_loss, valid_pose45_acc = losses.avg, top1.avg
    """@nni.report_intermediate_result(valid_pose45_acc)"""

    if valid_pose45_acc > best_acc:
        best_acc = valid_pose45_acc
    _print('>>>>>pose45_best_acc:{:.3f}------>>>>>'.format(best_acc))
    return best_acc


def valid_occlusion(model, val_loader, cfg, best_acc=0.0):
    _print = cfg.logger.print_log
    model.eval()
    epoch_time = TimeMeter()
    losses = AverageMeter(':.4f')
    ...
    _print(
        'Test: --Accuracy:{:6.2f} --Average Loss: {:.4f} --Time: {}'.format(top1.avg, losses.avg, epoch_time))
    valid_loss, valid_occlusion_acc = losses.avg, top1.avg
    """@nni.report_intermediate_result(valid_occlusion_acc)"""

    if valid_occlusion_acc > best_acc:
        best_acc = valid_occlusion_acc
    _print('>>>>>occlusion_best_acc:{:.3f}------>>>>>'.format(best_acc))
    return best_acc
...
def main():
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        if epoch % args.eval_freq == 0:
            best_pose_30 = valid_pose30(model, pose30_loader, cfg, best_acc=best_pose_30)
            best_pose_45 = valid_pose45(model, pose45_loader, cfg, best_acc=best_pose_45)
            best_pose_occlusion = valid_occlusion(model, occlusion_loader, cfg, best_acc=best_pose_occlusion)
            model.eval()
            epoch_time = TimeMeter()
            losses = AverageMeter(':.4f')
            top1 = AverageMeter(':6.2f')
