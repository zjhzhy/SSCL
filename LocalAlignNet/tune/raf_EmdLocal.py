...
import torch
import os
from LocalAlignNet.models.EmdLocal import DeepEMD
import argparse
import utils
...
from DataLoader.raf_scale import get_dataloader
import json
...
def main():
    ...
    parser.add_argument('--balance_factor', type=float, default=0.6)
    ...
    if 'balance' in args.tune:
        print('tuning balance_factor:{}'.format(args.balance_factor))
        """nni.variable(nni.choice(40,50,60,70), name=args.balance_factor)"""
        args.balance_factor = args.balance_factor
        logsuffix = logsuffix + 'balance_factor{:.2f}'.format(args.balance_factor)
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        for i, batch in pbar:
            ...
            # forward
            logit, emd_dists, target, weights = model(batch)

            # balance loss
            # weights shape:[batch,M]
            # wmean shape: [1,M]
            wmean = weights.mean(dim=0, keepdim=False)
            M = wmean.size(0)
            unionw = torch.full([M], 1. / M).cuda()
            balanceloss = torch.norm(wmean - unionw, p=1, dim=0)
            # loss computation
            ...
            for dist in emd_dists:
                dist = dist.mean(0)
                emd_loss += dist
            loss = cls_loss - emd_loss * args.alpha + args.balance_factor * balanceloss
        ...
        if epoch % args.eval_freq == 0:
            ...
            with torch.no_grad():
                ...
                for i, batch in pbar:
                    logit, _, target, weights = model(batch)
                    criterion = nn.CrossEntropyLoss()
...
if __name__ == '__main__':
    ...
    logger = logging.getLogger('DEMSearch_AutoML')
    try:
        main()
