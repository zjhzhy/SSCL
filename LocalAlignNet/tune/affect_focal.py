...
sys.path.append('/home/lab303/zhy/train/train/junhao/junhao_code/MoPro')
import torch.nn as nn
from torch.optim import lr_scheduler
import torch
import os
from LocalAlignNet.models.EANInter import DeepEMD
import argparse
import utils
import pandas as pd
...
import sys
import nni
from DataLoader.affect256_scale_excel import get_dataloader as get_dataloader_excel
from DataLoader.ferplus_argument import get_dataloader as get_dataloader_ferpluse
from PIL import Image, ImageFile
ImageFile.LOAD_TRUNCATED_IAMGES = True
def main():
    description = '负样本做focal，正样本不做，把同类别当做正样本_msceleb'
    # global
    ...
    parser.add_argument('--test_mode', type=str, default='valid')
    # optimizer
    parser.add_argument('--lr', type=float, default=0.01)
    parser.add_argument('--wd', type=float, default=0.0001)#0.0001
    ...
    parser.add_argument('--backbone', type=str, default='resnet18_msceleb')
    ...
    if 'gama' in args.tune:
        print('tuning gama:{}'.format(args.gama))
        """nni.variable(nni.quniform(0.8,1.2,0.1), name=args.gama)"""
        args.gama = float(args.gama)
        logsuffix = logsuffix + 'gama{}'.format(args.gama)
    # if 'lr' in args.tune:
    ...
    logsuffix += '' + str(nni.get_sequence_id())
    cfg = Config.cfg
    ...
    best_acc_pose45=0.0
    best_acc_pose30 = 0.0
    best_acc_occlusion = 0.0
    best_acc_epoch = 0
    best_acc_epoch_pose45 = 0
    best_acc_epoch_pose30 = 0
    best_acc_epoch_occlusion = 0
    # load models
    ...
    # load data
    train_loader, val_loader,pose30_loader,pose45_loader,occlusion_loader=get_dataloader_excel(args)
    #train_loader, val_loader, test_loader=get_dataloader_ferpluse(args)
    #pose30_loader, pose45_loader = get_dataloader_pose(args)
    #occlusion_loader=get_dataloader_oss(args)
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        for i, batch in pbar:
            ...
            # 计算不同类别为负样本，mm矩阵
            m = args.batch_size
            t1 = target.unsqueeze(0)
            t2 = target.unsqueeze(1)
            mask = t1.eq(t2.repeat(1, target.shape[0]))
            neg_mask = ~mask
            # diag_mask = torch.eye(m, dtype=torch.bool).to('cuda')
            # mask &= diag_mask
            # mask = ~mask
            emd_loss = 0.0
            gamma = float(args.gama)
            for dist in emd_dists:
                p = dist.softmax(dim=1)
                p = torch.pow(p, exponent=gamma)
                pdist = p * dist
                negative_dist = torch.masked_select(pdist, neg_mask)
                mean_negative_dist = negative_dist.mean(0)
                positive_dist = torch.masked_select(dist, mask)
                mean_positive_dist = positive_dist.mean(0)
                emd_loss = emd_loss - mean_positive_dist + mean_negative_dist
        ...
        if epoch % args.eval_freq == 0:
            ...
            if valid_acc > best_acc:
                ...
                if args.save_best and epoch > 0:
                    model_path = os.path.join(cfg.path.ckpt, 'best_{:03d}_{:.3f}.pth'.format(best_acc_epoch,best_acc))
                    torch.save({
                        'models': model.state_dict(),
                    }, model_path)
            ...
            with torch.no_grad():
                # pbar = tqdm(enumerate(BackgroundGenerator(test_loader)), total=len(test_loader))
                pbar = enumerate(BackgroundGenerator(pose45_loader))
            ...
            epoch_time.stop()
            _print(
                'Test_pose45: --Accuracy:{:6.2f} --Average Loss: {:.4f} --Time: {}'.format(top1.avg, losses.avg, epoch_time))
            valid_loss, valid_acc = losses.avg, top1.avg
            """@nni.report_intermediate_result(valid_acc)"""

            if valid_acc > best_acc_pose45:
                best_acc_pose45 = valid_acc
                best_acc_epoch_pose45 = epoch
                if args.save_best and epoch > 0:
                    model_path = os.path.join(cfg.path.ckpt, 'best')
                    torch.save({
                        'models': model.state_dict(),
                    }, model_path)
            _print('>>>>>best_acc_pose45:{:.3f}------best_acc_epoch:{:03d}>>>>>'.format(best_acc_pose45, best_acc_epoch_pose45))
            first_batch_gap_time = time.time()
            ...
            with torch.no_grad():
                # pbar = tqdm(enumerate(BackgroundGenerator(test_loader)), total=len(test_loader))
                pbar = enumerate(BackgroundGenerator(pose30_loader))
            ...
            epoch_time.stop()
            _print(
                'Test_pose30: --Accuracy:{:6.2f} --Average Loss: {:.4f} --Time: {}'.format(top1.avg, losses.avg,
                                                                                           epoch_time))
            valid_loss, valid_acc = losses.avg, top1.avg
            """@nni.report_intermediate_result(valid_acc)"""

            if valid_acc > best_acc_pose30:
                best_acc_pose30 = valid_acc
                best_acc_epoch_pose30 = epoch
                if args.save_best and epoch > 0:
                    model_path = os.path.join(cfg.path.ckpt, 'best')
                    torch.save({
                        'models': model.state_dict(),
                    }, model_path)
            _print('>>>>>best_acc_pose30:{:.3f}------best_acc_epoch:{:03d}>>>>>'.format(best_acc_pose30,
                                                                                        best_acc_epoch_pose30))
            first_batch_gap_time = time.time()
            ...
            with torch.no_grad():
                # pbar = tqdm(enumerate(BackgroundGenerator(test_loader)), total=len(test_loader))
                pbar = enumerate(BackgroundGenerator(occlusion_loader))
            ...
            epoch_time.stop()
            _print(
                'Test_occlusion: --Accuracy:{:6.2f} --Average Loss: {:.4f} --Time: {}'.format(top1.avg, losses.avg,
                                                                                           epoch_time))
            valid_loss, valid_acc = losses.avg, top1.avg
            """@nni.report_intermediate_result(valid_acc)"""

            if valid_acc > best_acc_occlusion:
                best_acc_occlusion = valid_acc
                best_acc_epoch_occlusion = epoch
                if args.save_best and epoch > 0:
                    model_path = os.path.join(cfg.path.ckpt, 'best')
                    torch.save({
                        'models': model.state_dict(),
                    }, model_path)
            _print('>>>>>best_acc_occlusion:{:.3f}------best_acc_epoch:{:03d}>>>>>'.format(best_acc_occlusion,
                                                                              best_acc_epoch_occlusion))
            first_batch_gap_time = time.time()
    ...
    return best_acc,best_acc_pose30,best_acc_pose45,best_acc_occlusion


if __name__ == '__main__':
    '''@nni.get_next_parameter()'''
    logger = logging.getLogger('affect_scale_invarient')
    try:
        main()
