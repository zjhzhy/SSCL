...
import torch
import os
from LocalAlignNet.models.L2AlignNet import L2Align
import argparse
import utils
import pandas as pd
...
def main():
    ...
    if 'alpha' in args.tune:
        ...
        """nni.variable(nni.choice(0.1,0.01,0.001,0.0001), name=args.alpha)"""
    ...
    best_acc_epoch = 0
    # load models
    model = L2Align(args).to(cfg.device)
    ...
    for epoch in range(0, 0 + args.max_epoch):
        ...
        for i, batch in pbar:
            ...
            # forward
            logit, l2_loss, target = model(batch)
