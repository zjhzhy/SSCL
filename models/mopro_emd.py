...
class MoPro(nn.Module):
    ...
    def __init__(self, base_encoder, args, width):
        ...
        # encoder
        self.encoder_q = base_encoder(num_class=args.num_classes, low_dim=args.low_dim, width=width, pretrained=True)
        # momentum encoder
        self.encoder_k = base_encoder(num_class=args.num_classes, low_dim=args.low_dim, width=width, pretrained=True)
    ...
    def forward(self, batch, args, is_eval=False, is_proto=False, is_clean=False):
        ...
        if is_proto:
            ...
            C = center.size(0)
            D = center.size(1)
            center = center.repeat(B, 1, 1)
            x = x.unsqueeze(1)
            x = x.repeat(1, C, 1)
            l2 = (x - center).pow(2)
            # shape:BC
            BC = l2.sum(dim=2, keepdim=False)
            d_sum = BC.sum(dim=1, keepdim=True)
            d_dot = BC.prod(dim=1, keepdim=True)
            r = d_dot / d_sum
            logits_proto = torch.div(r, BC)

            # sharppen
            logits_proto = logits_proto ** (1 / args.temperature)
            logtis_sum = logits_proto.sum(dim=1, keepdim=True)
            logits_proto = logits_proto / logtis_sum

        else:
            logits_proto = 0
        if is_clean:
            # noise cleaning
            soft_label = args.pseudo_w * F.softmax(output, dim=1) + (1 - args.pseudo_w) * F.softmax(logits_proto,
                                                                                                    dim=1)

            # keep ground truth label
            gt_score = soft_label[target >= 0, target]
            clean_idx = gt_score > (1 / args.num_classes)
